...
public class ModService(Config config, Dictionary<ulong, User> users)
{
    private readonly Config _config = config;
    private readonly Dictionary<ulong, User> _users = users;

    public async Task SilenceUser(SocketGuildUser user, string? reason) => await SilenceUser(new SocketGuildUserAdapter(user), reason);
    ...
    public static async Task AddRole(SocketGuildUser user, ulong roleId, string? reason)
        => await AddRole(new SocketGuildUserAdapter(user), roleId, reason);

    public static async Task AddRole(IIzzyGuildUser user, ulong roleId, string? reason)
        => await user.AddRoleAsync(roleId, reason is null ? null : new Discord.RequestOptions { AuditLogReason = reason });

    public static async Task RemoveRole(SocketGuildUser user, ulong roleId, string? reason)
        => await RemoveRole(new SocketGuildUserAdapter(user), roleId, reason);

    public static async Task RemoveRole(IIzzyGuildUser user, ulong roleId, string? reason)
        => await user.RemoveRoleAsync(roleId, reason is null ? null : new Discord.RequestOptions { AuditLogReason = reason });

    public static async Task AddRoles(SocketGuildUser user, IEnumerable<ulong> roles, string? reason)
        => await AddRoles(new SocketGuildUserAdapter(user), roles, reason);

    public static async Task AddRoles(IIzzyGuildUser user, IEnumerable<ulong> roles, string? reason)
